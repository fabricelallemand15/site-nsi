...
def define_env(env):
    ...
    def script(lang: str, nom: str, indentation=0, stop="") -> str:
        ...
        # par Franck Chambon
        sortie = []
        with open("docs/" + os.path.dirname(env.variables.page.url.rstrip('/')) + f"/{nom}", 'r', encoding="UTF-8") as f:
    ...
    def read_ext_file(nom_script : str, path : str, filetype : str = 'py') -> str:
        ...
        try: 
            if path == "":
                #print("path vide")
                try : # base case 
                    f = open(f"""{short_path}/scripts/{nom_script}.{filetype}""", encoding="UTF-8")
                except : # minor subcase : .py directly in docs/ (should not happen...)
                    f = open(f"""{short_path}/{path}/{nom_script}.{filetype}""", encoding="UTF-8")
            else:
                #print(path)
                #print(f"""{short_path}/{path}/{nom_script}.{filetype}""")
                f = open(f"""{short_path}/{path}/{nom_script}.{filetype}""", encoding="UTF-8")
    ...
    def create_upload_button(tc : str) -> str:
        ...
        nb_folders = len(convert_url_to_utf8(env.variables.page.abs_url).split('/'))-2
        path_img="../"*nb_folders
        path_img = path_img[:-1]
    ...
    def IDE(nom_script : str = '', mode : str = 'h', MAX : int = 5, SANS : str = "") -> str:
        ...
        if path_img == "":
            path_img = "."
        path_file = '/'.join(filter(lambda folder: folder != "", convert_url_to_utf8(env.variables.page.abs_url).split('/')[:-2]))
        print("***",'/'.join(filter(lambda folder: folder != "", convert_url_to_utf8(env.variables.page.abs_url).split('/')[:-2])), "***")
